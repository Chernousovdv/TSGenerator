...
def load_monash(
    ...
    normalize    : bool, default True
        ...
        и отбрасываем. Рекомендуется True при сравнении с генератором —
    ...
    # Отключаем проверку SSL сертификатов для обхода проблем с self-signed сертификатами
...
if __name__ == "__main__":
    ...
    Быстрый запуск из командной строки:
        python analyze_monash.py [--mode all|financial|diverse]
    
    Режимы:
        all         — все доступные датасеты Monash (по умолчанию)
        financial   — только финансовые и бизнес-датасеты
        diverse     — сбалансированная выборка из разных областей (быстрый анализ)
    
    Использует ОДИН и тот же набор датасетов для одномерных и многомерных
    статистик — это обеспечивает согласованность сравнения.
    """
    import sys
    
    # Определяем режим из аргументов командной строки
    mode = "all"  # режим по умолчанию
    if len(sys.argv) > 1:
        if sys.argv[1] in ("all", "financial", "diverse"):
            mode = sys.argv[1]
        else:
            print(f"Неизвестный режим: {sys.argv[1]}")
            print("Использование: python analyze_monash.py [--mode all|financial|diverse]")
            sys.exit(1)
    ...
    # ========================================================================
    # ВЫБОР НАБОРА ДАТАСЕТОВ ПО РЕЖИМУ
    # ========================================================================
    if mode == "financial":
        print("=" * 70)
        print("РЕЖИМ: ФИНАНСОВЫЕ И БИЗНЕС ДАТАСЕТЫ")
        print("=" * 70)
        # Только финансовые и бизнес-датасеты
        # Это дает более релевантное сравнение для финансовых задач
        ...
        dataset_config = {
            ...
            # Макроэкономика
            "fred_md": 200,
            # Бизнес/экономика (M4 competition)
            "m4_yearly": None,
            "m4_quarterly": None,
            "m4_monthly": None,
            "m4_weekly": None,
            "m4_daily": None,
            "m4_hourly": None,
            # Банковские операции (NN5 competition)
            "nn5_daily": 200,
            "nn5_weekly": 200,
            # Туризм (экономическая активность)
            "tourism_yearly": None,
        ...
        n_series_univariate = 300  # Больше рядов от каждого датасета
        n_series_mv = 150  # Больше рядов для многомерного анализа
        max_batches = 200  # Увеличено для большего числа сэмплов
    elif mode == "diverse":
        print("=" * 70)
        print("РЕЖИМ: СБАЛАНСИРОВАННАЯ ВЫБОРКА (DIVERSE)")
        print("=" * 70)
        # Сбалансированная выборка из разных областей для быстрого анализа
        # Включает ~13 датасетов из различных категорий
        dataset_config = {
            # Финансовые (2)
            "bitcoin": 200,
            "fred_md": 200,
            ...
            "tourism_monthly": None,
            # Энергетика (2)
            "australian_electricity_demand": 136,
            "london_smart_meters": 136,
            # Погода (2)
            "weather": 168,
            "saugeenday": 200,
            # Трафик (1)
        ...
        n_series_univariate = 300  # Умеренное число рядов
        n_series_mv = 100  # Для многомерного анализа
        max_batches = 100  # Ограниченное число батчей
    else:  # mode == "all"
        print("=" * 70)
        print("РЕЖИМ: ВСЕ ДАТАСЕТЫ")
        print("=" * 70)
        # Все доступные датасеты
        dataset_config = {
            # Стандартные датасеты — без нарезки
            "tourism_yearly": None,
            "tourism_quarterly": None,
            "tourism_monthly": None,
            "hospital": None,
            ...
            "bitcoin": 200,
            "pedestrian_counts": 200,
            # Высокочастотные датасеты — с нарезкой
            "traffic_hourly": 168,
            "kdd_cup_2018": 168,
            "oikolab_weather": 168,
            "australian_electricity_demand": 136,
            "london_smart_meters": 136,
            "solar_10_minutes": 108,
            "wind_farms": 108,
        }
        n_series_univariate = 500
        n_series_mv = 100
        max_batches = 100

    # ========================================================================
    # Определяем директорию для сохранения по режиму
    # ========================================================================
    mode_dir = f"results/monash_profile/{mode}"  # diverse/, financial/, all/
    
    # ========================================================================
    # 1. Одномерные статистики
    # ========================================================================
    print("\n" + "=" * 70)
    print("ШАГ 1: ОДНОМЕРНЫЕ СТАТИСТИКИ")
    print("=" * 70)
    
    df_univariate = profile_monash_multi(
        dataset_names=dataset_config,
        statistics=long_stats,
        n_series=n_series_univariate,
        normalize=True,
        save_dir=mode_dir,
    )
    
    # ========================================================================
    # 2. Многомерные метрики — НА ТЕХ ЖЕ ДАТАСЕТАХ
    # ========================================================================
    print("\n" + "=" * 70)
    print("ШАГ 2: МНОГОМЕРНЫЕ МЕТРИКИ")
    print("=" * 70)
    
    print("\nЗагрузка данных для многомерного анализа...")
    print(f"(Режим: {mode}, n_series={n_series_mv})")
    
    # Собираем все ряды из тех же датасетов
    all_series = []
    for ds_name, max_len in dataset_config.items():
        print(f"\n  Загрузка {ds_name}...")
        try:
            series = load_monash(
                ds_name,
                n_series=n_series_mv,
                min_length=50,
                max_length=max_len,
                normalize=True,
                seed=42,
            ...
            print(f"    Добавлено {len(series)} рядов")
        except Exception as e:
            print(f"    Ошибка: {e}")
    
    print(f"\n  Всего рядов: {len(all_series)}")
    
    # Для многомерного анализа нужны ряды одинаковой длины
    T_fixed = 100  # Фиксированная длина для всех
    series_mv = [s[:T_fixed] if len(s) >= T_fixed else s for s in all_series]
    series_mv = [s for s in series_mv if len(s) == T_fixed]
    
    print(f"  Рядов длиной {T_fixed}: {len(series_mv)}")
    
    # Формируем батч (B, T, D) — 4 канала на батч
    D_channels = 4
    B = len(series_mv) // D_channels
    B = min(B, max_batches)  # Увеличенное ограничение
    
    print(f"  Формируем {B} батчей по {D_channels} канала...")
    
    if B >= 4:
        mv_batches = []
        for b in range(B):
            batch_channels = []
            for d in range(D_channels):
                idx = b * D_channels + d
                if idx < len(series_mv):
                    s = series_mv[idx]
                    if len(s) == T_fixed:
                        batch_channels.append(s)
            if len(batch_channels) == D_channels:
                mv_batches.append(np.stack(batch_channels, axis=-1))
        
        if len(mv_batches) >= 4:
            mv_data = np.stack(mv_batches, axis=0)  # (B, T, D)
            print(f"  Форма батча: {mv_data.shape}")
            
            analyzer = MultivariateAnalyzer(seed=42)
            half = mv_data.shape[0] // 2
            
            results = analyzer.analyze_batch(
                synthetic=mv_data[:half],
                real=mv_data[half:half*2] if half*2 <= mv_data.shape[0] else mv_data[half:],
                signature_config=SignatureConfig(depth=2, backend="manual"),
                cross_corr_config=CrossCorrelationConfig(max_lag=5),
            )
            
            summary = analyzer.compute_summary(results)
            
            # Сохраняем summary в папку режима
            mv_df = pd.DataFrame([summary])
            mv_csv_path = Path(mode_dir) / "multivariate_metrics.csv"
            mv_df.to_csv(mv_csv_path, index=False)
            print(f"  Сохранено (summary): {mv_csv_path}")
            
            # Сохраняем raw данные в папку режима
            all_arrays = {}
            for key, arr in results.items():
                if isinstance(arr, np.ndarray) and arr.ndim == 1:
                    all_arrays[f"synthetic_{key}"] = arr
                    all_arrays[f"real_{key}"] = arr
                elif isinstance(arr, np.ndarray) and arr.ndim == 2:
                    for i in range(arr.shape[1]):
                        all_arrays[f"synthetic_{key}_{i}"] = arr[:, i]
                        all_arrays[f"real_{key}_{i}"] = arr[:, i]
            
            max_len = max((len(v) for v in all_arrays.values()), default=0)
            ...
            raw_csv_path = Path(mode_dir) / "multivariate_metrics_raw.csv"
            raw_df.to_csv(raw_csv_path, index=False)
            print(f"  Сохранено (raw): {raw_csv_path}")
            
            print(f"\nМНОГОМЕРНЫЕ МЕТРИКИ (B={B} батчей):")
            for key, value in summary.items():
                if isinstance(value, (int, float)):
                    print(f"  {key}: {value:.6f}")
        else:
            print("  Недостаточно батчей (нужно >= 4)")
    else:
        print("  Недостаточно данных (нужно >= 4 батчей)")
    
    print("\n" + "=" * 70)
    print(f"ГОТОВО. Результаты в: {mode_dir}/")
    print("=" * 70)
    print(f"\nРежим: {mode}")
    print("Важно: ОДИН и тот же набор датасетов использовался для:")
    print("  - одномерных статистик (ADF, энтропия, сезонность и др.)")
    print("  - многомерных метрик (сигнатуры, кросс-корреляции, площади Леви)")
    print("=" * 70)
    print(f"\nСтруктура результатов:")
    print(f"  {mode_dir}/")
    print(f"    ├── monash_all.png           — гистограммы одномерных статистик")
    print(f"    ├── monash_all.csv           — сырые данные одномерных статистик")
    print(f"    ├── multivariate_metrics.csv — агрегированные многомерные метрики")
    print(f"    └── multivariate_metrics_raw.csv — сырые многомерные данные")
    print("=" * 70)
